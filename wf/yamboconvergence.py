...
from aiida.workflows.user.cnr_nano.yambo_utils import default_step_size, update_parameter_field
...
class YamboConvergenceWorkflow(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input("threshold", valid_type=Float)
    ...
    def init_parameters(self,paging):
        # set initial values for all converge parameters
        # this can handle only simple values, for all the rest, we need to have it done
        # case by case. 
    ...
    def start(self):
        # construct yambo workflow calculation/s submit them
        outs={}
        ...
        # when done r0 will be  {'gw': <ParameterData: uuid: 0d49adbe-87d6-49dd-bcc6-729d49056710 (pk: 228)>}
    ...
    def is_not_converged(self):
        ...
        # check the the  differences are minimal 
    ...
    def get_total_range(self,node_id):
        ...
        argwlk = np.argwhere(table[:,0]==float(lowest_k))
        argwlb = np.argwhere(table[:,1]==float(lowest_b))
        argwhb = np.argwhere(table[:,1]==float(highest_b))
        arglb = np.intersect1d(argwlk,argwlb)
        arghb = np.intersect1d(argwlk,argwhb)
