...
class YamboWorkflow(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input(
            "calculation_set_pw", valid_type=ParameterData
        ...
        spec.input(
            "gamma_pw", valid_type=Bool, default=Bool(0), required=False)
        ...
        spec.input(
            ...
            default=Bool(0))
        spec.input(
            "to_set_bands", valid_type=Bool, required=False, default=Bool(0))
        spec.input("bands_groupname", valid_type=Str, required=False)
        spec.outline(cls.start_workflow,
                     while_(cls.can_continue)(cls.perform_next, ),
                     cls.report_wf)
        #spec.dynamic_output()
    ...
    def start_workflow(self):
        ...
        if 'previous_yambo_workchain' in list(self.inputs.keys()):
            ...
            if 'scf_remote_folder' in list(wf_outs.get_outputs_dict().keys()):
            ...
            if 'nscf_remote_folder' in list(wf_outs.get_outputs_dict().keys()):
            ...
            if 'yambo_remote_folder' in list(
                    wf_outs.get_outputs_dict().keys()):
        ...
        if self.ctx.last_step_kind == 'yambo' and self.ctx.yambo_res:
            ...
            try:
                self.ctx.yambo_pks.append(
                    self.ctx.yambo_res.out.gw.get_dict()["yambo_pk"])
            except AttributeError:
