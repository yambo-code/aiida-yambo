...
def generate_yambo_input_params(precodename, yambocodename, parent_folder,
    ...
    if parent_calc.process_type=='aiida.calculations:quantumespresso.pw':
        ...
        if 'SE_CPU' not in list(edit_parameters.keys()):
...
def reduce_parallelism(typ, roles, values, calc_set):
    ...
    num_mpiprocs_per_machine = calculation_set['resources'][
        'num_mpiprocs_per_machine']
    omp_threads = 1
    if 'environment_variables' in list(calculation_set.keys()):
        omp_threads = calculation_set['environment_variables'].pop(
            'OMP_NUM_THREADS', 1)
    ...
    if typ == 'X_all_q_CPU':
        ...
        if False:
            if num_mpiprocs_per_machine < calculation_set['resources'][
                    'num_mpiprocs_per_machine']:
                c = c / 2
        ...
        if False:
            if num_mpiprocs_per_machine < calculation_set['resources'][
                    'num_mpiprocs_per_machine'] and v > 1:
                v = v / 2
            if num_machines > calculation_set['resources']['num_machines']:
                c = c * 2
    ...
    if typ == 'SE_CPU':
        ...
        try:
            ...
            b = 1
        if num_mpiprocs_per_machine < calculation_set['resources'][
                'num_mpiprocs_per_machine'] and qp > 1:
            qp = qp / 2
        if num_mpiprocs_per_machine < calculation_set['resources'][
                'num_mpiprocs_per_machine'] and qp == 1:
            qp = qp / 2
...
def update_parameter_field(field, starting_point, update_delta):
    ...
    if field in ['PPAPntXp', 'NGsBlkXp', 'BSENGBlk', 'BSENGexx',
                 'FFTGvecs']:  # single numbers
        ...
        raise WorkflowInputValidationError(
            "convergences the field {} are not supported".format(field))
...
def set_default_qp_param(parameter=None):
    ...
    if 'ppa' not in list(edit_param.keys()):
        edit_param['ppa'] = True
    if 'gw0' not in list(edit_param.keys()):
        edit_param['gw0'] = True
    if 'HF_and_locXC' not in list(edit_param.keys()):
        edit_param['HF_and_locXC'] = True
    if 'em1d' not in list(edit_param.keys()):
        edit_param['em1d'] = True
    if 'DysSolver' not in list(edit_param.keys()):
        edit_param['DysSolver'] = "n"
    if 'Chimod' not in list(edit_param.keys()):
        edit_param['Chimod'] = "Hartree"
    if 'LongDrXp' not in list(edit_param.keys()):
    ...
    if 'rim_cut' not in list(edit_param.keys()):
    ...
    if 'SE_CPU' not in list(edit_param.keys()):
        edit_param['SE_CPU'] = "1 2 4"
        edit_param['SE_ROLEs'] = "q qp b"
    if 'X_all_q_CPU' not in list(edit_param.keys()):
        edit_param['X_all_q_CPU'] = "1 1 4 2"
        edit_param['X_all_q_ROLEs'] = "q k c v"
    if 'FFTGvecs' not in list(edit_param.keys()):
