...
# Updated  and converted to calcfunction on February 21st, 2022 by Miki Bonacci (miki.bonacci@unimore.it)
#
import numpy as np
import os, shutil
import datetime
...
def gw2wannier90(
    ...
    seedname = seedname.value #argv[1]  # for instance "silicon"
    seednameGW = seedname + ".gw"  #for instance "silicon.gw"
    targets = options.get_list() #[s.lower() for s in argv[2:]]  #options read from command line
    output_path = output_path.value
    
    path_nnkp = output_path
    path_mmn_amn = output_path
    
    if nnkp_file: path_nnkp = nnkp_file._repository._repo_folder.abspath+'/path/'
    if pw2wannier_parent: path_mmn_amn = pw2wannier_parent._repository._repo_folder.abspath+'/path/' 
    ...
    f_raw = open(output_path+seedname + '.gw2wannier90.raw', 'w')
    #Opening seedname.nnkp file
    f = open(path_nnkp+seedname + ".nnkp", "r")
    ...
    feig_out = open(output_path+seednameGW + ".eig", "w")
    for ik in range(NKPT):
    ...
    if calcAMN:
        try:
            print("----------\n AMN module  \n----------")
            f_amn_out = open(output_path+seednameGW + ".amn", "w")
            f_amn_in = open(path_mmn_amn+seedname + ".amn", "r")
    ...
    if calcMMN:
        try:
            ...
            f_mmn_out = open(os.path.join(output_path+seednameGW + ".mmn"), "w")
            f_mmn_in = open(os.path.join(path_mmn_amn+seedname + ".mmn"), "r")
