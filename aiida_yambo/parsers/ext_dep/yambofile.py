...
class YamboFile():
    ...
    def __init__(self, filename, folder='.'):
        ...
        self.data = {
        }  #dictionary containing all the important data from the file
        self.kpoints = {}
        self.timing = []
        self.timing_section = []
        ...
        if any(
            ...
            if 'EELS' in title:
                ...
                filename.startswith(prefix)
                for prefix in self._report_prefixes):
            self.type = 'report'
        elif any(filename.startswith(prefix) for prefix in self._log_prefixes):
            self.type = 'log'
        elif any(
                filename.startswith(prefix)
                for prefix in self._p2y_log_prefixes):
            self.type = 'p2y_log'
        elif any(
                filename.startswith(prefix)
                for prefix in self._netcdf_prefixes) and _has_netcdf:
    ...
    def parse_output(self):
        ...
        if self.type == "output_gw":
            tags = [
                line.replace('(meV)', '').replace('Sc(Eo)', 'Sc|Eo')
                for line in self.lines
                if all(tag in line for tag in ['K-point', 'Band', 'Eo'])
            ][0]
            ...
            if table.ndim > 1:
                k_index = [str(int(i))
                           for i in table[:, 0]]  # first column  has kpoints
            else:
            ...
            for ind in range(len(k_index)):
                for itag in range(len(tags)):
                    if k_index[ind] not in list(_kdata.keys()):
                        _kdata[k_index[ind]] = {}
                    try:
                        _kdata[k_index[ind]][tags[itag]].append(
                            table[ind, itag])
                    except KeyError:
                        try:
                            _kdata[k_index[ind]][tags[itag]] = [
                                table[ind, itag]
                            ]
                        except IndexError:
        ...
        if self.type in ["output_absorption", "output_eels"]:
            ...
            tags = [
                tag.strip() for tag in re.findall(
                    '#\n#\s+((?:(?:[`0-9a-zA-Z\-\/\|\\(\)\_[\]]+)\s+)+)#\n\s',
                    string)[0].split()
            ]
            tags = [
                tag.replace('-', '_').replace('/', '_').split('[')[0]
                for tag in tags
            ]
    ...
    def parse_netcdf_gw(self):
        ...
        if _has_netcdf:
            ...
            data['qp_table'] = qp_table[:,
                                        1:]  # ib, ik, ,(isp if spin polarized)
            #qpoints
    ...
    def parse_report(self):
        ...
        kpoints = re.compile(
            '^  [A-X*]+\sK\s\[([0-9]+)\]\s[:](?:\s+)?([0-9.E-]+\s+[0-9.E-]+\s+[0-9.E-]+)\s[A-Za-z()\s*.]+[0-9]+[A-Za-z()\s*.]+([0-9.]+)'
        )
        timing = re.compile(
            '\s+?[A-Za-z]+iming\s+?[A-Za-z/\[\]]+[:]\s+?([a-z0-9-]+)[/]([a-z0-9-]+)[/]([a-z0-9-]+)'
        )
        timing_section = re.compile('^\s+?\[\d+[.]?\d+?\]\s+?(.*)$')
        game_over = re.compile(
            '^\s+?\[\d+\]\s+?G\w+\s+?O\w+\s+?\&\s+?G\w+\s+?\w+'
        )  # Game over & Game summary
        p2y_complete = re.compile(
            '^(\s+)?[-<>\d\w]+\s+?P\d+[:]\s+?==\s+?P2Y\s+?\w+\s+?==(\s+)?'
        )  # P2Y Complete
        p2y_complete_v2 = re.compile(
            '(\s+)?[-<>\w\d]+(\s+)?==(\s+)?P2Y(\s+)?\w+(\s+)?=='
        )  # P2Y Complete
        yambo_wrote = re.compile('(?:\s+)?[[]WR[./\w]+[]](?:[-])+')
        timing_overview = re.compile(
            '\s+?([a-zA-Z_()0-9\s]+)\s+?:\s+?([0-9a-z.]+)\s*?CPU\s*?([(]?\s*?(\d+)?\s*?[a-z]+[,]\s*?([\d.]+\s+?s)\s+?avg[)])?$'
        )
        ...
        for line in self.lines:
            ...
            if timing_overview.match(line):
                name, total, _, calls, avg = timing_overview.match(
                    line).groups()
                self.timing_overview[name.replace(
                    ' ', '')] = self.get_seconds(total)
                self.timing_overview[name.replace(' ', '') +
                                     '_calls'] = int(calls) if calls else calls
                if avg:
                    self.timing_overview[name.replace(' ', '') +
                                         '_avg'] = self.get_seconds(
                                             avg.replace(' ', ''))

        full_lines = ''.join(self.lines)
        qp_regx = re.compile(
            '(^\s+?QP\s\[eV\]\s@\sK\s\[\d+\][a-z0-9E:()\s.-]+)(.*?)(?=^$)',
            re.M | re.DOTALL)
        kp_regex = re.compile(
            '^\s+?QP\s\[eV\]\s@\sK\s\[(\d+)\][a-z0-9E:()\s.-]+$')
        spliter = re.compile(
            '^(B[=]\d+\sEo[=]\s+?[E0-9.-]+\sE[=]\s+?[E0-9.-]+\sE[-]Eo[=]\s+?[E0-9.-]+\sRe[(]Z[)][=]\s+?[*E0-9.-]+\sIm[(]Z[)][=]\s?[*E0-9.-]+\snlXC[=]\s+?[E0-9.-]+\slXC[=]\s+?[E0-9.-]+\sSo[=]\s+?[E0-9.-]+)'
        )
        extract = re.compile(
            'B[=](\d+)\sEo[=](?:\s+)?([E0-9.-]+)\sE[=](?:\s+)?([E0-9.-]+)\sE[-]Eo[=](?:\s+)?([E0-9.-]+)\sRe[(]Z[)][=](?:\s+)?([*E0-9.-]+)\sIm[(]Z[)][=](?:\s+)?[*E0-9.-]+\snlXC[=](?:\s+)?([E0-9.-]+)\slXC[=](?:\s+)?([E0-9.-]+)\sSo[=](?:\s+)?([E0-9.-]+)'
        )
            ...
            for line in each:  # different band indexes =B
                if kp_regex.match(line):
                    ...
                    for qp_data in data_lines:
                        ...
                        listed = [
                            bindex, dft_energy, qp_energy, qp_correction,
    ...
    def get_seconds(self, time_string):
        ...
        if time[0].endswith('h'):
            return int(float(time[0].replace("h", ""))) * 3600 + int(
                float(time[1].replace("m", ""))) * 60 + int(
                    float(time[2].replace("s", "")))
        elif time[0].endswith('m'):
            return int(float(time[0].replace("m", ""))) * 60 + int(
                float(time[1].replace("s", "")))
    ...
    def parse_log(self):
        ...
        if not hasattr(self, 'lines'):
            with open('%s/%s' % (self.folder, self.filename)) as fl:
                self.lines = fl.readlines()
        warning = re.compile(
            '^\s+?<([0-9a-z-]+)> ([A-Z0-9]+)[:] \[(WARNING)\]? ([a-zA-Z0-9\s.()\[\]]+)?'
        )
        error = re.compile(
            '^\s+?<([0-9a-z-]+)> ([A-Z0-9]+)[:] \[(ERROR)\]? ([a-zA-Z0-9\s.()\[\]]+)?'
        )
        self.warnings.extend(
            [line for line in self.lines if warning.match(line)])
        self.errors.extend([line for line in self.lines if error.match(line)])
        memory = re.compile(
            '^\s+?<([0-9a-z-]+)> ([A-Z0-9]+)[:] \[M  ([0-9.]+) Gb\]? ([a-zA-Z0-9\s.()\[\]]+)?'
        )
        self.memstats.extend(
            [line for line in self.lines if memory.match(line)])
        if len(self.memstats) > 0:
            self.max_memory = max([
                float(memory.match(line).groups()[2]) for line in self.memstats
            ])
            self.last_memory = [
                float(memory.match(line).groups()[2]) for line in self.memstats
            ][-1]
        ...
        self.timestats.extend([
            self.get_seconds(times.match(line).groups()[0])
            for line in self.lines if times.match(line)
        ])
        self.last_time = self.timestats[-1]
        if len(self.memstats) > 0:
            self.last_memory_time = [
                self.get_seconds(memory.match(line).groups()[0])
                for line in self.memstats
            ][-1]

        generic_error = re.compile(
            '^(?=\s+)?([A-Z0-9]+)[:] \[(ERROR)\](?=\s+)?([a-zA-Z0-9\s.()\[\]]+)?'
        )
        paralle = re.compile(
            '^(?=\s+)?([A-Z0-9]+)[:] \[ERROR\](?=\s+)?Impossible(?=\s+)?(?=[a-zA-Z0-9\s.()\[\]]+)?'
        )
        unphysical = re.compile(
            '^(?=\s+)?([A-Z0-9]+)[:] \[ERROR\](?=\s+)?\[NetCDF\]\s*NetCDF[:]\s*NC_UNLIMITED\s*in\s*the\s*wrong\s*index'
        )
        self.errors.extend([
            line for line in self.lines if (generic_error.match(line) and (
                paralle.match(line) or unphysical.match(line)))
        ])
        for line in self.lines:
    ...
    def parse_p2y_log(self):
        ...
        yambo_wrote = re.compile(
            '(?:\s+)?(?:[<>\w\d]+)(:?\s+)?(?:P\d+[:])(?:\s+)?(?:[[]\w+[]])?(?:\s+)?Writing(?:\s+)?\w+(?:\s+)?'
        )
        for line in self.lines:
            if p2y_complete.match(line) or p2y_complete_v2.match(line):
                self.p2y_complete = True
        ...
        self.errors.extend([
            ...
            if (generic_error.match(line) or error.match(line))
        ])
    ...
    def __str__(self):
        return "type: %9s   file: %s/%s" % (self.type, self.folder,
                                            self.filename)
