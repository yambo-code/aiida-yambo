...
def read_intp_out(jsondata, legendpos, label, output="fullconv"):
    ...
    for data  in full_result['ordered_step_output']:
        if "kpoints" in data["convergence_space"].keys():
        ...
        if "FFTGvecs" in data["convergence_space"].keys():
            indep_fft = np.array(data['convergence_space']['FFTGvecs'] )
            mn,mx= indep_fft[0], indep_fft[-1]
            dep_fft = np.array(data['energy_widths'] )
            independent_ax = np.arange( independent_ax[-1],  independent_ax[-1] +  indep_fft.size )
            plotter(fig,gs, ax1, independent_ax , dep_fft, "FFT {}-{} Ry".format(mn,mx)  )
            mx = np.amax(dep_fft)
            mn = np.amin(dep_fft)
            min_dependent, max_dependent = get_extremes(mn, mx,min_dependent ,max_dependent )

        if "GbndRnge" in data["convergence_space"].keys():
        ...
        if "NGsBlkXp" in data["convergence_space"].keys():
