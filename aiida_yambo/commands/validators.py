...
def validate_node(callback_kwargs, ctx, param, value):
    ...
    if not isinstance(worknode, WorkCalculation):
        raise click.BadParameter('node<{}> is not of type {}'.format(value, WorkCalculation.__name__))

    try:
        plotdata = worknode.get_outputs_dict()['result'].get_dict()
    except KeyError as exception:
        raise click.BadParameter('Node {} does node have result key:  {}, FullConvergence workflow may not have successfully completed'.format(value, exception))
...
def validate_legendpos(callback_kwargs, ctx, param, value):
    ...
    if value not in range(0,11):
        raise click.BadParameter('Legend position needs to be in range [0-10]'.format(value))
